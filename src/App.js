...
class App extends Component {
  ...
  componentDidMount() {
   axios.get('/api/messages').then(res => this.setState({allMessages: res.data}))
  }

  createMessage() {
    const {username, message} = this.state
    axios.post('/api/messages', {username, message}).then(res => this.setState({allMessages: res.data}))
  }
